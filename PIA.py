...
def verificar_login():
    ...
    if resultado:
        ...
        if bcrypt.checkpw(contrasena, contrasena_hash):
            ...
            messagebox.showinfo("Login exitoso", f"¡Bienvenido, {nombre_departamento}!")
            mostrar_frame("menu")
        else:
...
def preparar_requisicion():
    ...
    id_requisicion = f"{parte_nombre}{str(contador).zfill(8)}"
...
def agregar_al_carrito():
    ...
    carrito.append((seleccion.split(" - ")[0], cantidad))
    actualizar_carrito()
    combo_opciones.set("")
